    ...
    if let Some(current_pid) = scheduler.current_pid {
        if let Some(current) = scheduler.processes.get_mut(&current_pid) {
            ...
            core::arch::asm!(
                ...
                out(reg) current.cpu_state.rsi,
                out(reg) current.cpu_state.rdi,
                out(reg) current.cpu_state.rbp,
                out(reg) current.cpu_state.r8,
                out(reg) current.cpu_state.r9,
                out(reg) current.cpu_state.r10,
                out(reg) current.cpu_state.r11,
                out(reg) current.cpu_state.r12,
                out(reg) current.cpu_state.r13,
                out(reg) current.cpu_state.r14,
                out(reg) current.cpu_state.r15,
            );
        }
    }
    ...
    if let Some(next_pid) = scheduler.schedule() {
        if let Some(next) = scheduler.processes.get(&next_pid) {
            ...
            core::arch::asm!(
                ...
                in(reg) next.cpu_state.rsi,
                in(reg) next.cpu_state.rdi,
                in(reg) next.cpu_state.rbp,
                in(reg) next.cpu_state.r8,
                in(reg) next.cpu_state.r9,
                in(reg) next.cpu_state.r10,
                in(reg) next.cpu_state.r11,
                in(reg) next.cpu_state.r12,
                in(reg) next.cpu_state.r13,
                in(reg) next.cpu_state.r14,
                in(reg) next.cpu_state.r15,
            );
        }
    }
}
